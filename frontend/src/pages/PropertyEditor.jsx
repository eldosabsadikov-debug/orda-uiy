...
export default function PropertyEditor({ mode }) {
  ...
  async function handleUpload(event) {
    ...
    try {
      ...
      const next = [...form.imagesText.split("\n").filter(Boolean), ...(data.urls || [])].join("\n");
      update("imagesText", next);
    } finally {
      setUploading(false);
    }
  }
  ...
  if (loading) return <section className="page-section"><div className="container"><p>{t("common.loading")}</p></div></section>;
      ...
      <div className="container editor-wrap">
        ...
        <form className="editor-form" onSubmit={handleSubmit}>
          ...
          <fieldset>
            ...
            <label><span>{t("form.title")}</span><input value={form.title} onChange={(e) => update("title", e.target.value)} required /></label>
            <label className="full"><span>{t("form.description")}</span><textarea value={form.description} onChange={(e) => update("description", e.target.value)} required /></label>
          </fieldset>

          <fieldset>
            <legend>{t("form.location")}</legend>
            <label><span>{t("form.price")}</span><input type="number" value={form.price} onChange={(e) => update("price", e.target.value)} required /></label>
            <label><span>{t("filters.city")}</span><input value={form.city} onChange={(e) => update("city", e.target.value)} required /></label>
            <label><span>{t("filters.district")}</span><input value={form.district} onChange={(e) => update("district", e.target.value)} required /></label>
            <label><span>{t("form.address")}</span><input value={form.address} onChange={(e) => update("address", e.target.value)} /></label>
          </fieldset>
          ...
          <fieldset>
            ...
            <label className="full"><span>{t("form.extraNotes")}</span><textarea value={form.extraNotes} onChange={(e) => update("extraNotes", e.target.value)} /></label>
          </fieldset>

          <fieldset>
            <legend>{t("form.media")}</legend>
            <label className="full"><span>{t("form.imageUrls")}</span><textarea value={form.imagesText} onChange={(e) => update("imagesText", e.target.value)} placeholder="https://..." /></label>
            <label className="full file-label"><span>{t("form.upload")}</span><input type="file" accept="image/*" multiple onChange={handleUpload} />{uploading && <em>{t("common.loading")}</em>}</label>
          </fieldset>

          <fieldset>
            <legend>{t("form.contacts")}</legend>
            <label><span>{t("form.phone")}</span><input value={form.phone} onChange={(e) => update("phone", e.target.value)} /></label>
            <label className="full"><span>{t("form.otherContacts")}</span><textarea value={form.otherContacts} onChange={(e) => update("otherContacts", e.target.value)} /></label>
          </fieldset>

          <div className="form-actions">
            <button className="yellow-btn" type="submit" disabled={saving}>{saving ? t("common.loading") : t("common.save")}</button>
            <Link className="ghost-btn" to="/admin">{t("common.cancel")}</Link>
          </div>
        </form>
      </div>
    </section>
