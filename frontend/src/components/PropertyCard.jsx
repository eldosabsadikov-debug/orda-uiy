...
export default function PropertyCard({ property }) {
  ...
  return (
    <article className="property-card">
      <div className="property-image-wrap" aria-label={title}>
        <img className="property-image" src={images[imageIndex]} alt={title || t("common.noImage")} />
        {hasManyImages && (
          <>
      ...
      <div className="property-body">
        ...
        <Link className="yellow-link" to={`/apartments/${property.id}`}>{t("common.more")}</Link>
      </div>
    </article>
  );
}
